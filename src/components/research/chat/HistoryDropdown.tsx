...
import { getGuestChats } from '@/lib/guestStorage';
...
const HistoryDropdown = () => {
  ...
  return (
    <Popover className="relative">
      ...
      >
        <PopoverPanel className="absolute right-0 mt-2 w-80 origin-top-right rounded-2xl bg-popover border border-border shadow-xl z-50">
          <div className="p-3">
              ...
              <div className="space-y-1 max-h-96 overflow-y-auto">
                {chats.map((chat) => (
                  <Link
                    key={chat.id}
                    href={`/c/${chat.id}`}
                    className="block px-3 py-2 rounded-xl hover:bg-secondary transition-colors duration-200"
                  >
                    <div>
