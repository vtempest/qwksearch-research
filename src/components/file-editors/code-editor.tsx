...
const getLangExtension = (langKey: string): any => {
  try {
      ...
      console.warn(
        ...
        `Looking for similar: ${
          availableLangs
            .filter(
              (l) =>
                l.includes(langKey.toLowerCase()) ||
                langKey.toLowerCase().includes(l),
            )
            .join(", ") || "none"
        }`,
    ...
    console.error(
      `[CodeEditor] Error loading language extension "${langKey}":`,
      error,
    );
    return null;
  }
...
export function CodeEditor({
  ...
  const langExtension = useMemo(() => {
    try {
      ...
      if (!langFn || typeof langFn !== "function") {
        if (process.env.NODE_ENV === "development") {
          console.warn(
            `[CodeEditor] No language function found for "${language}"`,
          );
        }
        return [];
      }
      ...
      if (process.env.NODE_ENV === "development") {
        console.warn(
          `[CodeEditor] Language extension for "${language}" returned null/undefined`,
        );
      }
      return [];
    } catch (error) {
      console.error(
        `[CodeEditor] Failed to load language extension for "${language}":`,
        error,
      );
      return [];
    }
  ...
  useEffect(() => {
    ...
    const hasNoLocalChanges =
      readOnly || localContent === savedContent.current || !localContent;
