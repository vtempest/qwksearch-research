...
export function UnifiedKbEntryModal({
  ...
  const folderValidation = useNameValidation(
    newFolderName,
    "folder",
    existingFolderNames,
  );
  ...
  const handleFileUpload = async () => {
    ...
    try {
          ...
          if (response.ok) {
            ...
            if (response.status === 413) {
              try {
                ...
                errorMessage =
                  errorData.detail || "Knowledge base limit (50MB) exceeded";
              } catch {
      ...
      if (completedFiles === selectedFiles.length) {
        ...
        toast.success(
          `Uploaded ${completedFiles} of ${selectedFiles.length} files`,
        );
      } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.filename_changed) {
          toast.info(
            `File was renamed to "${result.final_filename}" to avoid conflicts`,
          );
        }
  ...
  const handleGitClone = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/knowledge-base/folders/${selectedFolder}/clone-git-repo`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${session.token}`,
  ...
  const selectedFolderInfo = folders.find(
          ...
          <div className="space-y-6 p-1">
            ...
            <div className="space-y-3">
              <Label className="text-sm font-medium">
                Destination Folder{" "}
                {!selectedFolder && <span className="text-red-500">*</span>}
              </Label>
              ...
              {!folderValidation.isValid && newFolderName && (
                <p className="text-sm text-red-600">
                  {folderValidation.friendlyError}
                </p>
              )}
            </div>
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Supports PDF, DOC, TXT, MD, CSV, and more • Max 50MB
                        total
                      </p>
                    </div>
                  </div>
              ...
              <TabsContent value="text" className="space-y-4 mt-6">
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      {!filenameValidation.isValid && filename && (
                        <p className="text-sm text-red-600">
                          {filenameValidation.friendlyError}
                        </p>
                      )}
                      <p className="text-xs text-muted-foreground">
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      This content will be saved as a searchable text file in
                      your knowledge base
                    </p>
                  </div>
                </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              {activeTab === "upload" && (
                <Button
                  ...
                  disabled={
                    !selectedFolder || selectedFiles.length === 0 || isUploading
                  }
                  className="gap-2"
                >
                  {isUploading ? (
