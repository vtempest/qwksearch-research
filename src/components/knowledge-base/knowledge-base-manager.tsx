...
import {
  ...
  type Folder,
  type Entry,
} from "@/hooks/knowledge-base/use-folders";
...
export function KnowledgeBaseManager({
  ...
  React.useEffect(() => {
    const buildTree = () => {
      const tree: TreeItem[] = folders.map((folder) => {
        const existingFolder = treeData.find(
          (item) => item.id === folder.folder_id,
        );
        ...
        return {
          ...
          data: folder,
          children:
            folderEntries[folder.folder_id]?.map((entry) => ({
              id: entry.entry_id,
              type: "file" as const,
              name: entry.filename,
  ...
  const fetchFolderEntries = async (folderId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/knowledge-base/folders/${folderId}/entries`,
        {
          headers: {
            Authorization: `Bearer ${session.token}`,
            "Content-Type": "application/json",
  ...
  const handleExpand = async (folderId: string) => {
    ...
    setTreeData((prev) =>
      prev.map((item) =>
        item.id === folderId ? { ...item, expanded: !item.expanded } : item,
      ),
    );
  ...
  const getFolderSelectionState = (folderId: string) => {
    ...
    const selectedCount = folderEntryIds.filter((id) =>
  ...
  const handleEditChange = (newName: string) => {
    ...
    const existingNames = folders
      ...
      .filter(
        (name) =>
          name !== folders.find((f) => f.folder_id === editingFolder)?.name,
      );
  ...
  const handleFinishEdit = async () => {
    ...
    const nameValidation = FileNameValidator.validateName(
      trimmedName,
      "folder",
    );
    const hasConflict =
      nameValidation.isValid &&
      FileNameValidator.checkNameConflict(trimmedName, existingNames);
    const isValid = nameValidation.isValid && !hasConflict;
  ...
  const handleSaveSummary = async (newSummary: string) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/knowledge-base/entries/${editSummaryModal.fileId}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${session.token}`,
      ...
      if (response.ok) {
        ...
        const fileItem = treeData
          .flatMap((folder) => folder.children || [])
          .find((file) => file.id === editSummaryModal.fileId);
  ...
  const handleMoveFile = async (fileId: string, targetFolderId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/knowledge-base/entries/${fileId}/move`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${session.token}`,
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const activeItem = treeData
      .flatMap((folder) => [folder, ...(folder.children || [])])
      .find((item) => item.id === activeItemId);
    const overItem = treeData
      .flatMap((folder) => [folder, ...(folder.children || [])])
      .find((item) => item.id === overItemId);
    ...
    try {
      ...
      for (let i = 0; i < fileArray.length; i++) {
        ...
        if (!validation.isValid) {
          toast.error(
            `Invalid filename "${file.name}": ${FileNameValidator.getFriendlyErrorMessage(file.name, "file")}`,
          );
          continue;
        }
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (result.filename_changed) {
              toast.info(
                `File "${result.original_filename}" was renamed to "${result.final_filename}" to avoid conflicts`,
              );
            }
          } else {
            if (response.status === 413) {
              try {
                ...
                toast.error(
                  `Knowledge base limit exceeded: ${errorData.detail || "Total file size limit (50MB) exceeded"}`,
                );
              } catch {
                toast.error(
                  "Knowledge base limit exceeded: Total file size limit (50MB) exceeded",
                );
              }
            } else {
              toast.error(
                `Failed to upload ${file.name}: Error ${response.status}`,
              );
            }
          }
        } catch (error) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {enableAssignments
                ? `Manage ${agentName}'s knowledge sources and access`
                : headerDescription}
            </p>
          </div>
          <UnifiedKbEntryModal
                ...
                {treeData.map((item) => {
                    ...
                    treeData.forEach((folder) => {
                      if (folder.children) {
                        folder.children.forEach((child) => {
                          allAssignments[child.id] = selectedEntries.has(
                            child.id,
                          );
                        });
                      }
                  ...
                  return (
                    <SharedTreeItem
                      ...
                      assignments={
                        enableAssignments ? allAssignments : undefined
                      }
                      assignmentIndeterminate={
                        enableAssignments ? allIndeterminateStates : undefined
                      }
                      onToggleAssignment={
                        enableAssignments
                          ? (id) => {
                              const targetItem =
                                treeData.find((f) => f.id === id) ||
                                treeData
                      ...
                      validationError={
                        editingFolder === item.id ? validationError : null
                      }
      ...
      <KBDeleteConfirmDialog
        ...
        onClose={() =>
          setDeleteConfirm({ isOpen: false, item: null, isDeleting: false })
        }
