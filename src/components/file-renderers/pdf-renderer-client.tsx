...
export default function PdfRendererClient({
    url,
    className,
    compact = false,
}: PdfRendererProps) {
    ...
    const pageWidth =
        containerWidth > 0 ? Math.max(containerWidth - 48, 100) : undefined;
    ...
    return (
        <div
            ref={containerRef}
            className={cn(
                "flex flex-col w-full h-full bg-muted/20 overflow-hidden",
                className,
            )}
            style={{ contain: "strict" }}
        >
