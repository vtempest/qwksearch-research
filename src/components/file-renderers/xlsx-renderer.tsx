...
import { Search, FileSpreadsheet, Filter, ChevronDown } from "lucide-react";
  ...
  const sheets = await Promise.all(
    sheetNames.map(async (sheetName) => {
      try {
        ...
        console.error(
          `[XlsxRenderer] Error parsing sheet "${sheetName}":`,
          err,
        );
        return { headers: [], data: [] };
      }
...
export function XlsxRenderer({
  ...
  const [parsed, setParsed] = useState<ParsedWorkbook>({
    sheets: [],
    sheetNames: [],
  });
    ...
    async function load() {
      try {
        ...
        if (typeof xlsxPath === "string" && xlsxPath.startsWith("blob:")) {
          ...
          const url = new URL(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/sandboxes/${resolvedSandboxId}/files/content`,
          );
  ...
  const visibleHeaders = currentSheet.headers.filter(
    (h) => !hiddenColumns.has(h),
  );
